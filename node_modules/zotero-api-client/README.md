...
const response = await localapi.library('user', 0).items().get();
```

Write requests require two extra pieces:

1. **Server ID**: every write must carry a `Zotero-Server-ID` header identifying the Zotero instance. Every local API response includes this header, so it can be bootstrapped with a bare root request.
2. **Local API key**: requested with `local().authorize()`, which makes Zotero prompt the user to allow write access for your app.
...
If the user picks "Always allow", `auth.isRemembered()` returns `true` and the key remains valid until revoked in Zotero's settings. Otherwise the key is single-use: it is consumed by the first write request, and a write with a used or unknown key fails with `401`, at which point authorization should be requested again.
...
await api(key, { apiScheme: 'http', apiAuthorityPart: 'localhost:23119', apiPath: 'api/' })
	.library('user', 0).serverID(serverID).items().post([ /* items */ ]);
```

Note: the local API drops requests that appear to come from a browser (a `Mozilla/` user agent or an `Origin` header) unless the `Zotero-Allowed-Request` header is present; set it via the `zoteroAllowedRequest` option.
...
* [zotero-api-client](#module_zotero-api-client)
    * [~ApiResponse](#module_zotero-api-client..ApiResponse)
        ...
        * [.getServerID()](#module_zotero-api-client..ApiResponse+getServerID) ⇒ <code>string</code>
    ...
    * [~AuthorizeResponse](#module_zotero-api-client..AuthorizeResponse) ⇐ <code>ApiResponse</code>
        * [.getResponseType()](#module_zotero-api-client..AuthorizeResponse+getResponseType)
        * [.getKey()](#module_zotero-api-client..AuthorizeResponse+getKey) ⇒ <code>string</code>
        * [.isRemembered()](#module_zotero-api-client..AuthorizeResponse+isRemembered) ⇒ <code>boolean</code>
    ...
    * [~ErrorResponse](#module_zotero-api-client..ErrorResponse) ⇐ <code>Error</code>
        ...
        * [.getServerID()](#module_zotero-api-client..ErrorResponse+getServerID) ⇒ <code>string</code>
    ...
    * [~api()](#module_zotero-api-client..api) ⇒ <code>Object</code>
        ...
        * [~fulltext()](#module_zotero-api-client..api..fulltext) ⇒ <code>Object</code>
        ...
        * [~serverID(serverID)](#module_zotero-api-client..api..serverID) ⇒ <code>Object</code>
        ...
        * [~local()](#module_zotero-api-client..api..local) ⇒ <code>Object</code>
        * [~authorize(appName)](#module_zotero-api-client..api..authorize) ⇒ <code>Object</code>
...
<a name="module_zotero-api-client..ApiResponse+getServerID"></a>

#### apiResponse.getServerID() ⇒ <code>string</code>
...
**Kind**: instance method of [<code>ApiResponse</code>](#module_zotero-api-client..ApiResponse)  
**Returns**: <code>string</code> - ID of the Zotero instance that produced the response  
...
**See**: [getResponseType](#module_zotero-api-client..ApiResponse+getResponseType)  
<a name="module_zotero-api-client..AuthorizeResponse"></a>

### zotero-api-client~AuthorizeResponse ⇐ <code>ApiResponse</code>
Represents a response to a local API authorization request (POST /api/local/authorize).
Local API only
...
<a name="module_zotero-api-client..AuthorizeResponse+getResponseType"></a>

#### authorizeResponse.getResponseType()
**Kind**: instance method of [<code>AuthorizeResponse</code>](#module_zotero-api-client..AuthorizeResponse)  
**See**: [getResponseType](#module_zotero-api-client..ApiResponse+getResponseType)  
<a name="module_zotero-api-client..AuthorizeResponse+getKey"></a>

#### authorizeResponse.getKey() ⇒ <code>string</code>
Local API key granted by the user, to be sent in the "Zotero-API-Key" header on
	  subsequent write requests

**Kind**: instance method of [<code>AuthorizeResponse</code>](#module_zotero-api-client..AuthorizeResponse)  
**Returns**: <code>string</code> - Local API key authorizing write requests  
<a name="module_zotero-api-client..AuthorizeResponse+isRemembered"></a>

#### authorizeResponse.isRemembered() ⇒ <code>boolean</code>
...
**Kind**: instance method of [<code>AuthorizeResponse</code>](#module_zotero-api-client..AuthorizeResponse)  
**Returns**: <code>boolean</code> - Whether the key is persistent  
...
**Returns**: <code>number</code> - Version of the content in response  
<a name="module_zotero-api-client..ErrorResponse+getServerID"></a>

#### errorResponse.getServerID() ⇒ <code>string</code>
Value of the "Zotero-Server-ID" header in response if present. Identifies the Zotero
	  instance that served the request when using the local API
...
**Chainable**  
**Returns**: <code>Object</code> - Partially configured api functions  
<a name="module_zotero-api-client..api..fulltext"></a>

#### api~fulltext() ⇒ <code>Object</code>
...
<a name="module_zotero-api-client..api..serverID"></a>

#### api~serverID(serverID) ⇒ <code>Object</code>
...
instance that produced it in a "Zotero-Server-ID" header (see [getServerID](#module_zotero-api-client..ApiResponse+getServerID)) and write requests
...
| Param | Type | Description |
| --- | --- | --- |
| serverID | <code>String</code> | Server ID previously obtained from a local API response |
...
**Chainable**  
**Returns**: <code>Object</code> - Partially configured api functions  
<a name="module_zotero-api-client..api..local"></a>

#### api~local() ⇒ <code>Object</code>
...
**Chainable**  
**Returns**: <code>Object</code> - Partially configured api functions  
<a name="module_zotero-api-client..api..authorize"></a>

#### api~authorize(appName) ⇒ <code>Object</code>
...
| Param | Type | Description |
| --- | --- | --- |
| appName | <code>String</code> | Name of the calling application, displayed to the                            user in the authorization prompt |
...
| Param | Type | Description |
| --- | --- | --- |
| keysToDelete | <code>Array</code> | An array of keys to delete. Depending on                                how api has been configured, these will                                be item keys, collection keys, search                                keys or tag names. If not present, api                                should be configured to use specific                                item, collection, saved search or settings                                key, in which case, that entity will be deleted.                                Tag names are joined into a single ' || '-separated                                query parameter, hence a tag name that contains                                ' || ' cannot be deleted (see zotero/dataserver#118) |
| opts | <code>Object</code> | Optional api configuration. If duplicate,                         overrides properties already present. For a list                         of all possible properties, see documentation                         for request() function |
...
| config.zoteroServerID | <code>String</code> | 'Zotero-Server-ID' header. Used with the local API only |
| config.zoteroAllowedRequest | <code>String</code> | 'Zotero-Allowed-Request' header. Used with the local API only; Zotero's local server drops requests that carry a browser-like user agent (or an Origin header) unless this header is present |
...
| config.resource.local | <code>String</code> | use 'local' resource. Local API only |
| config.resource.authorize | <code>String</code> | use 'authorize' resource. Local API only |
| config.resource.fulltext | <code>String</code> | use 'fulltext' resource |
| config.method | <code>String</code> | forwarded to fetch() |
| config.body | <code>String</code> | forwarded to fetch() |
| config.appName | <code>String</code> | name of the calling application, used to construct the body of a local API authorization request. Local API only. Incompatible with "body" |
